...
import { setCluesDivActive } from "./Buttons";
...
function Input(props) {
    ...
    const processGuessSubmit = (guess, year) => {
        if (years.indexOf(guess) < 0) {
            inputNotifier.alert(
                "Input must be a valid year in the format XXXX e.g. 1999 between 1970-2020"
            );
            return;
        }
    ...
    const handleYearSubmit = (e) => {
        ...
        if (years.indexOf(input) < 0 && input !== "") {
            inputNotifier.alert(
                "Input must be a valid year in the format XXXX e.g. 1999 between 1970-2020 or blank"
            );
            setCluesDivActive();
            return;
        }
    ...
    const checkInputIsNumber = (e) => {
        ...
        if (!isNaN(e.target.value.charCodeAt(0))) {
            if (
                e.target.value[len - 1].charCodeAt(0) < 48 ||
                e.target.value[len - 1].charCodeAt(0) > 57
            ) {
                inputNotifier.alert(
                    "Input must be a valid year in the format XXXX e.g. 1999"
                );
                return false;
            }
        }
    ...
    return (
        <div>
            ...
            <div className="year-input">
                <div
                    className="year-input-content"
                    onMouseEnter={showYearInputToolTip}
                >
                    <input
                        type="number"
                        name="year"
            ...
            <div className="guess-input">
                <div
                    className="guess-input-content"
                    onMouseEnter={showGuessInputTooltip}
                >
                    <input
                        type="number"
                        name="guess"
                ...
                <Button
                    ...
                    color="primary"
                    id="guess-button"
                    onClick={() => processGuessSubmit(guess, year)}
                    disabled={
                        year === "" || loaded === true || disable === true
                    }
                >
                    Submit
                </Button>
